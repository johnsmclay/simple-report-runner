$(document).ready(function() {
	...
	var shiftDates = {
		...
		// @returns Object beginDate and endDate
		getDifferentMonth : function(direction,month,year) {
			...
			var dates = {
				beginDate 	: monthsArray[month] + '/01/' + year,
				endDate		: monthsArray[month] + '/' + days + '/' + year
			};
		...
		getThisMonth : function() {
			var date = new Date();
			var dates = {
				beginDate 	: monthsArray[date.getMonth()] + '/01/' + date.getFullYear(),
				endDate		: monthsArray[date.getMonth()] + '/' + (date.getDate() - 1) + '/' + date.getFullYear()
			};
			
			return dates;
		},
		
		// Gets the current quarter up to the current date
		getThisQuarter : function() {
			var date = new Date();
			var quarter = Math.floor(date.getMonth() / 3);
			var firstDate = new Date(date.getFullYear(), quarter * 3,1);
			var dates = {
				beginDate 	: monthsArray[firstDate.getMonth()] + '/01/' + firstDate.getFullYear(),
				endDate		: monthsArray[date.getMonth()] + '/' + (date.getDate() - 1) + '/' + date.getFullYear()
			};
		...
		// Gets the previous quarter from the date entered into the input box
		getPreviousQuarter : function(month,year) {
			...
			var dates = {
				beginDate	: firstDate.toLocaleDateString(),
				// beginDate	: monthsArray[firstDate.getMonth()] + '/01/' + firstDate.getFullYear(),
				endDate		: finishDate.toLocaleDateString()
			};
			
			return dates;
		},
		
		// Gets the current Fiscal year up to the current date (current date is actually previous day, since no data exists for the current date)
		getFiscalYear : function() {
			...
			if (month >= 0 && month <= 5) {
				...
				var dates = {
					beginDate	: firstDate.toLocaleDateString(),
					endDate		: date.getMonth() + '/' + (date.getDate() - 1) + '/' + date.getFullYear()
				};
				...
				success	: function(response) {
					...
					if(response.status == false) {
						$('#subSchoolList') // Remove all options and reset the Select
							...
							$.each(response, function(key,val) { // Load all returned data into the Select
	...
	// Set up event listeners for all date adjusting buttons
	if ($('#previousMonth')) {
	...
	// Main funciton for Get Report button on gale page
	function galeFunction() {
		var action = $(this).attr('report');
		$.ajax({
			url		: 'gale/startGale',
			type	: 'POST',
			dataType: 'json',
			success : function(response) {
				if (response.status == true) {
					$('#secretIFrame').attr("src","http://localhost/dash/newdash/gale/downloadGale");
				}
			}
		});
	}
